...
void PlantsCard::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    if(event->button() == Qt::LeftButton)
    {
        ...
        if(!this->isEnoughSunshine || this->isCooldown)
        {
            std::unique_ptr<GameMedia> notEnough(new GameMedia(":/res/Media/sunshineNotEnoughMusic.mp3",this));
            return;
        }
        //阳光充足
        else
        {
            std::unique_ptr<GameMedia> choosePlant(new GameMedia(":/res/Media/choosePlant.wav"));
            //冷却
            this->coolDown();
            //减少阳光总数
